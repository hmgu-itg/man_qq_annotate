...
#  print(mp)
  ...
  manual.labels$peak_col=as.character(manual.labels$peak_col)
    #print(sapply(manual.labels$peak_col, FUN=is.na) | apply(manual.labels$peak_col, FUN=is.null))
   # manual.labels$peak_col=col2rgb(manual.labels$peak_col)
...
#  print(manual.labels)
  ...
  for ( i in chrs ) {
    ...
    # Also, not sure on the (i-1)*unknown.var, this looks like some sort
    # of offset that needs to be bigger as we move up the genome
    labpos[i]=offset+(max(t[[i]])-min(t[[i]]))/2+((i-1)*unknown.var)
    ...
    for( j in 1:(length(h$counts)) ){
      ...
      newx[posi:(posi+l-1)]=rep(h$mids[j], l)+((i-1)*unknown.var)
      ...
      if (length(addendum[addendum>sig.thresh])>0) {
        ...
        autopeaks=rbindlist(list(autopeaks,
                                 as.list(
                                   c(i, # chr
                                     subpos[subpval==max(subpval)], # pos
                                     10^(-1*(subpval[subpval==max(subpval)])), #pval
                                     h$mids[j]+((i-1)*unknown.var) # xcoord
                                     ) ) ) )
      }
      ...
      if(is.null(manual.labels)==FALSE) {
        ...
        if(nrow(manual.labels[labels.in.grid,])>0){
          ...
          if (nrow(mt)>1) {
            warning("[WARNING] Peaks too close to label separately!")
            mt=mt[1,]
          }

        # Initialise a ycoord column, this is used in the cases where we 
        # want to add, specific points of interest to the plot
        manual.labels[labels.in.grid, ycoord:=subset[subsetx%in%manual.labels[labels.in.grid, pos]]]
        ...
        manual.labels[labels.in.grid, xcoord:=h$mids[j]+((i-1)*unknown.var)]
